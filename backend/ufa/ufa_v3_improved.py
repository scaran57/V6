...
class UFADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # label
        real = r.get('real_score') or r.get('score') or r.get('actual', {})
        label = -1
        if isinstance(real, dict):
            # Format: {"home": 2, "away": 1} ou {"home_goals": 2, "away_goals": 1}
            h = real.get('home') or real.get('home_goals')
            a = real.get('away') or real.get('away_goals')
...
class UFAv3(nn.Module):
    def __init__(self, n_teams: int, n_leagues: int, emb_dim: int = 32, hidden: int = 128, dropout: float = 0.2):
        ...
        league_emb_dim = max(8, emb_dim//2)
        self.fc_in = nn.Linear(emb_dim*2 + league_emb_dim + 2, hidden)
        self.act = nn.ReLU()
...
def evaluate_on_dataset(path: str, batch_size: int = 128):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            correct += (preds[mask]==labels[mask]).sum().item()
            total += mask.sum().item()
    print(f'Eval accuracy: {correct}/{total} = {correct/max(1,total):.4f}')
